...
개발자·디자이너·프론트엔드가 매일 쓰는 유틸리티 43종을 한 페이지에 모은 **순수 정적 웹 도구 모음**입니다.
...
> 💡 상단바 검색(단축키 `/` 또는 `Ctrl/Cmd+K`)으로 도구를 빠르게 찾을 수 있습니다.
...
### 포맷 & 변환
- **JSON 포매터/뷰어** — JSON을 붙여넣고 Pretty(들여쓰기 2/4/tab)·Minify·Validate. 문법 오류 시 줄/열 위치를 알려주고, 접을 수 있는 트리 뷰를 제공합니다.
...
- **JSON ↔ CSV 변환** — 객체 배열 ↔ CSV/TSV. 따옴표·개행 셀 안전 처리, 타입 추론 옵션.
- **JSON Diff** — 키 순서를 무시하고 구조 기준으로 비교해 추가/삭제/변경을 경로와 함께 표시.
- **cURL → 코드 변환** — curl 명령을 fetch/axios 코드로. 미지원 플래그는 무시 목록으로 안내.
...
- **Markdown 미리보기** — 자체 구현 서브셋 렌더러(XSS 안전). 실시간 미리보기 + HTML 출력.
- **문자열 자르기** — 앞/뒤 N자만 남기기, N자 버리기, 범위(N~M번째) 추출. 줄마다 적용·말줄임표 옵션. 코드 포인트 기준이라 이모지가 쪼개지지 않습니다.
- **Slug 생성기** — 제목 → URL slug. 한글은 개정 로마자 표기법 기준 변환 또는 제거.

### 색상 & 디자인
- **색상 변환기** — HEX(`#RGB`/`#RRGGBB`/`#RRGGBBAA`) ↔ RGB(A) ↔ HSL(A) 실시간 상호 변환 + OKLCH 출력 + 프리뷰 스와치.
...
- **파비콘 생성기** — 이미지 → 16~512px PNG 세트 + favicon.ico(자체 ICO 빌더) + HTML 스니펫.
...
- **HTTP 레퍼런스** — 상태 코드·MIME 타입 사전, 통합 검색.
- **Chmod 계산기** — 755 ↔ rwxr-xr-x 상호 변환, setuid/setgid/sticky 지원.
